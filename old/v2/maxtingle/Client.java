...
public class Client
{
    ...
    public void sendMessage(Message msg) throws IOException {
        ...
        Main.debug("Client", "Sending message " + msg.message + " encoded to " + msg.convertToJson());
    ...
    public Message getMessage() throws Exception {
        ...
        Main.debug("Server", "Got message " + line);
    ...
    public void listenForReplies() {
        ...
        this._replyListener = new Thread(new Runnable()
        {
            @Override
            public void run() {
                Main.debug("Client", "Listening for replies");
                ...
                while(isReady()) {
                    try {
                        ...
                        Main.debug("Client", "Received reply " + json);
                        Message reply = Message._fromJson(json);
                        Main.debug("Client", "Received reply text " + reply.message);
                    }
                    catch(Exception e) {
                        Main.debug("Client", "Failed to read reply from server socket " + e.toString());
                    }
                }
